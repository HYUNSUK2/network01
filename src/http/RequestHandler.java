...
//			os.write( "<h1>이 페이지가 잘 보이면 실습과제 SimpleHttpServer를 시작할 준비가 된 것입니다.</h1>".getBytes( "UTF-8" ) );
	...
	private void reponse400Error(OutputStream os, String string) throws IOException {
		consoleLog("in resonse400Error method");
		File file = new File( DOCUMENT_ROOT + "/" + "error/400.html" );
		String mimeType = Files.probeContentType( file.toPath() );
		byte[] body = Files.readAllBytes( file.toPath() );
		
		os.write( (string + " 400 bad\r\n").getBytes( "UTF-8" ) );
	...
	private void response404Error(OutputStream os, String protocol) throws IOException {
		consoleLog("in resonse404Error method");
		File file = new File( DOCUMENT_ROOT + "/" + "error/404.html" );
		String mimeType = Files.probeContentType( file.toPath() );
		byte[] body = Files.readAllBytes( file.toPath() );
		
		os.write( (protocol + " 404 bad\r\n").getBytes( "UTF-8" ) );
		os.write( ("Content-Type:" + mimeType + "; charset=utf-8\r\n").getBytes( "UTF-8" ) );
		os.write( "\r\n".getBytes() );
